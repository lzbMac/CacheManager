...
extern NSString * const TTSCacheManagerObject;                  //返回Dictionary
extern NSString * const TTSCacheManagerSetObjectNotification;   //缓存存数据
extern NSString * const TTSCacheManagerGetObjectNotification;   //缓存取数据
extern NSString * const TTSCacheManagerRemoveObjectNotification;//移除缓存

@interface TTSCacheManager : NSObject

/** 默认缓存管理器 */
+ (TTSCacheManager *)defaultManager;

/** nil suite means use the default search list that +defaultManager uses
 *  magic: suitename name end with ".document" then save data to Document(NSDocumentDirectory)
 */

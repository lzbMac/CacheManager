...
#import "TTSEncrypt.h"
const NSString * TTSCacheStorageDefaultFinderName = @"storagefile";

@interface TTSCacheStorage ()
...
@end
@implementation TTSCacheStorage
...
- (TTSCacheStoreObject *)objectForKey:(NSString *)aKey {
    TTSCacheStoreObject *object = [self.storageCaches objectForKey:aKey];
...
- (void)removePermanentObjects {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSString *permanentPath = [self cachePathWithFinderName:self.finderNames[TTSCacheStorageObjectIntervalAllTime]];
        [TTSCacheStorage enumerateFilesWithPath:permanentPath usingBlock:^(NSString *fileName) {
...
- (void)removeDefaultObjectsWithCompletionBlock:(void (^)(long long folderSize))completionBlock {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSString *path = [self cachePathWithFinderName:self.finderNames[TTSCacheStorageObjectIntervalDefault]];
        __block long long folderSize = 0;
        [TTSCacheStorage enumerateFilesWithPath:path usingBlock:^(NSString *fileName) {
...
- (void)removeExpireObjects {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSString *path = [self cachePathWithFinderName:self.finderNames[TTSCacheStorageObjectIntervalTiming]];
        [TTSCacheStorage enumerateFilesWithPath:path usingBlock:^(NSString *fileName) {
            NSString *filePath = [path stringByAppendingPathComponent:fileName];
            BOOL isDir;
            if ([[NSFileManager defaultManager] fileExistsAtPath:filePath isDirectory:&isDir] && !isDir) {
...
+ (void)removeDefaultObjectsWithCompletionBlock:(void (^)(long long folderSize))completionBlock {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        TTSCacheStorage *storage = [TTSCacheStorage defaultStorage];
        NSString *path = [storage cachePathWithFinderName:storage.finderNames[TTSCacheStorageObjectIntervalDefault]];
        __block long long folderSize = 0;
        [TTSCacheStorage enumerateFilesWithPath:path usingBlock:^(NSString *fileName) {
            NSString *filePath = [path stringByAppendingPathComponent:fileName];
            long long size = [[[NSFileManager defaultManager] attributesOfItemAtPath:filePath error:nil] fileSize];
...
+ (void)removeExpireObjects {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        TTSCacheStorage *storage = [TTSCacheStorage defaultStorage];
        NSString *path = [storage cachePathWithFinderName:storage.finderNames[TTSCacheStorageObjectIntervalTiming]];
...
+ (void)removeExpireObjectsWithPath:(NSString *)path {
    [TTSCacheStorage enumerateFilesWithPath:path usingBlock:^(NSString *fileName) {
        ...
        BOOL isDir;
        if ([[NSFileManager defaultManager] fileExistsAtPath:filePath isDirectory:&isDir] && !isDir) {
            [[TTSCacheStorage defaultStorage] unarchiveObjectWithPath:filePath];
        }
        else {
            [TTSCacheStorage removeExpireObjectsWithPath:filePath];
        }
    }];
}

#pragma mark - archive/unarchive
- (void)archiveObject:(TTSCacheStoreObject *)object {
    @synchronized(self) {
...
- (TTSCacheStoreObject *)unarchiveObjectWithPath:(NSString *)path {
    ...
    switch (object.storageInterval) {
        ...
        case TTSCacheStorageObjectIntervalDefault:
        case TTSCacheStorageObjectIntervalAllTime: {
            return object;
        }   break;
    }
    return nil;
...
- (NSString *)filePathWithObject:(TTSCacheStoreObject *)object {
...
- (NSString *)cachePathWithFinderName:(NSString *)finderName {
    ...
    NSString *directory = isDocument ? [TTSCacheStorage documentDirectory] : [TTSCacheStorage cachesDirectory];
    NSString *fileDirectory = [NSString stringWithFormat:@"%@%@/", directory, [TTSCacheStorageDefaultFinderName md5]];
...
+ (NSString *)cachesDirectory {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{ instance = [TTSCacheStorage pathWithSearchDirectory:NSCachesDirectory]; });
    return instance;
}
+ (NSString *)documentDirectory {
    static NSString *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{ instance = [TTSCacheStorage pathWithSearchDirectory:NSDocumentDirectory]; });
    return instance;
}

...
- (NSData *)tcc_AESEncryptWithPassphrase:(NSString *)pass keybits:(AESKeyBits)kbs
{
	...
	int nrounds = tcc_rijndaelSetupEncrypt(rk, key, KEYBITS);
	...
	while (1) 
	{
		...
		tcc_rijndaelEncrypt(rk, nrounds, plaintext, ciphertext);
...
- (NSData *)tcc_AESDecryptWithPassphrase:(NSString *)pass keybits:(AESKeyBits)kbs
{
	...
	int nrounds = tcc_rijndaelSetupDecrypt(rk, key, KEYBITS);
	unsigned char *srcBytes = (unsigned char *)[self bytes];
	int index = 0;
	while (index < [self length])
	{
		...
		tcc_rijndaelDecrypt(rk, nrounds, ciphertext, plaintext);
